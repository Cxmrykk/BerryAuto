...
void handle_touch_event(const com::andrerinas::headunitrevived::aap::protocol::proto::InputReport& report) {
    if (report.has_touch_event() && video_streamer != nullptr) {
        ...
        if (mapped_x > video_streamer->get_desktop_width()) mapped_x = video_streamer->get_desktop_width();
        if (mapped_y > video_streamer->get_desktop_height()) mapped_y = video_streamer->get_desktop_height();

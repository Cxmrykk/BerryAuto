...
int main()
{
    ...
    while (usb_transport.is_running())
    {
        ...
        {
            // 1. REMOVE the channel_id == 0 filter so we can see TouchEvents and Pings
            ...
            if (frame.channel_id == 0)
            {
                ...
                if (!(frame.flags & FLAG_ENCRYPTED)) 
                {
                    ...
                    // 2. THE FIX: Handle AuthComplete in the CLEARTEXT block!
                    else if (msg_type == 4) // AuthComplete (From Head Unit)
                    {
                        ...
                        std::vector<uint8_t> auth_pt = {0x00, 0x04, 0x08, 0x00}; // Type 4, status = 0 (SUCCESS)
                        ...
                        // Immediately trigger Service Discovery
                ...
                else if (tls_finished && (frame.flags & FLAG_ENCRYPTED))
                {
                    ...
                    { 
                        ...
                        if (video_ch_id != -1) {
                            ...
                            GalFrame chan_frame;
                            chan_frame.channel_id = video_ch_id; // MUST be sent on the target channel

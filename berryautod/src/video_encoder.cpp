...
#define IOCTL_OR_FAIL(fd, req, arg, msg)                                                                               \
    ...
    {                                                                                                                  \
        std::cerr << "[V4L2-ERR] " << msg << " failed: " << strerror(errno) << "\n";                                   \
        return false;                                                                                                  \
    }
...
void VideoEncoderThread::start(int width, int height)
{
    ...
    if (!init_drm_capture())
    {
        std::cout << "[HW-WARN] No active Linux Desktop found (Headless Mode?). Falling back to internal Test Pattern "
                     "Generator."
                  << std::endl;
    ...
    for (int i = 0; i < 3; i++)
    {
        ...
        for (int c = 0; c < res->count_crtcs; c++)
        {
            ...
            if (crtc && crtc->buffer_id)
            {
                ...
                if (fb)
                {
                    std::cout << "[DRM-DEBUG] Found active framebuffer on " << path << " (Res: " << fb->width << "x"
                              << fb->height << ")" << std::endl;
                    ...
                    {
                        drm_mapped_buffer =
                            (uint8_t*)mmap(0, drm_buffer_size, PROT_READ, MAP_SHARED, drm_fd, map_req.offset);
...
bool VideoEncoderThread::init_v4l2_encoder()
{
    ...
    ctrls[1].value = 4000000;
    ctrls[2].id = V4L2_CID_MPEG_VIDEO_H264_I_PERIOD;
    ctrls[2].value = 30;
    ctrls[3].id = V4L2_CID_MPEG_VIDEO_REPEAT_SEQ_HEADER;
    ctrls[3].value = 1;
    ...
    v4l2_in_buffer = (uint8_t*)mmap(NULL, v4l2_in_len, PROT_READ | PROT_WRITE, MAP_SHARED, v4l2_fd,
                                    buf_out.m.planes[0].m.mem_offset);
    ...
    v4l2_out_buffer = (uint8_t*)mmap(NULL, v4l2_out_len, PROT_READ | PROT_WRITE, MAP_SHARED, v4l2_fd,
                                     buf_cap.m.planes[0].m.mem_offset);
        ...
        if (ioctl(v4l2_fd, VIDIOC_DQBUF, &buf_cap) == 0)
        {
            ...
            if (!nalu.empty())
            {
                ...
                while (offset < full_plaintext.size())
                {
                    ...
                    std::vector<uint8_t> chunk(full_plaintext.begin() + offset,
                                               full_plaintext.begin() + offset + chunk_size);
...
void VideoEncoderThread::cleanup_hardware()
{
    if (v4l2_fd >= 0)
    {
        ...
        ioctl(v4l2_fd, VIDIOC_STREAMOFF, &type);
        if (v4l2_in_buffer)
            munmap(v4l2_in_buffer, v4l2_in_len);
        if (v4l2_out_buffer)
            munmap(v4l2_out_buffer, v4l2_out_len);
        close(v4l2_fd);
    }
    if (drm_fd >= 0)
    {
        if (drm_mapped_buffer)
            munmap(drm_mapped_buffer, drm_buffer_size);
        close(drm_fd);
    }
}